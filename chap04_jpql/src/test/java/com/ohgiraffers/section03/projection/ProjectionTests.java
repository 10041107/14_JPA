...
public class ProjectionTests {
    ...
    /*
    프로젝션(projection)
    SELECT절에 조회할 대상을 지정하는 것을 ㅡ로젝션이라고 한다.
    (SELECT {프로젝션 대상} FROM)
    ...
    2. 임베디드 타입 프로젝션(임베디드 타입에 대한 설명은 MENUinfo클래스에서 설명)
    엔티티와 거의 비슷하게 사용되며 조회의 시작점이 될 수 있다. <=from절에서 사용 불가
    ...
    4. new명령어를 활용한 프로젝션
    다양한 종류의 단순 값들을 dto로 바로 조회하는 방식으로 new패키지명.dto명을 쓰면
    해당 dto로 바로 반환 받을 수 있다.
    new명령어를 사용하나 클래스의 객체는 엔티티가 아니므로 영속성 컨텍스트에서 관리되지 않는다.
     */
    ...
    void 양방향_연관관계_엔티티_프로젝션_테스트(){
        ...
        BiDirectionCategory categoryOfMenu = entityManager.createQuery(jpql,BiDirectionCategory.class)
                .setParameter("menuCode", menuCodeParamenter)
                .getSingleResult();
...
//        Assertions.assertNotNull(categoryOfMenu.getMenuList());
    ...
    @Test
    void TypedQuery를_이용한_스칼라_타입_프로덕션_테스트(){
        //when
    ...
    /*
    조회하려는 컬럼 값이 1개인 경우 tTypedQuery로 변환 타입을 단일 값에 대해 지정할 수 있지만 다중 열 컬럼을 조회하는 경우 타입을 지정하지 못한다.
    그떄 TypeQuery대신 query를 사용하여 object[]로 쉘의 정보를 반환 받아 사용한다.
     */

    @Test
    void query를_이용한_스칼라_타입_프로덕션_테스트(){
        //when
