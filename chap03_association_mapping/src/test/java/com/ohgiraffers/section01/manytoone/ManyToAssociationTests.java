...
public class ManyToAssociationTests {

    /*
    Association Mapping은 Entity클래스 간의 관계를 매핑하는 것을 의미한다.
    이를 통해 객체를 이용해 데이터베이스의 테입르 간의 관게를 매핑할 수 있다.

    다중성에 의한 분류
    연관 관계가 있는 객체 관계에서느 실제로 연관을 가지는 객체의 순에 따라 분류된다.
    ...
    ManyTOOne은 다수의 엔터티가 하나의 엔티티를 참조하는 상황에서 상용된다.
    예를 들어 하나의 카테고리가 여러 개의 메뉴를 가질 수 있는 상황에서 메뉴 엔티티가 카테고리 엔티티를 참조하는 것이다
    이때 메뉴 엔티티가 Many, 카테고리 엔티티가 One이 된다

    연관 관계를 가지는 엔티티를 조회하는 방법은 객체 그래프 탐색(객체 연관관계를 사용한 조회), 객체 지향 쿼리(jpql)사용이 있다
     */
    ...
    void 다대일_연관관계_객체_그래프_탐색을_이용한_조회_테스트() {
        ...
        System.out.println("menuCategoryCode : " + menuCategory);

    }
    /*
    jpql은 java Persistence Query Language의 약자로, 객체 지향 쿼리 언어 중 하나이다.
   객체 지향 모델에 맞게 작성된 쿼를 통해, 엔티티 객체를 대상으로 검색, 검색 결과를 토대로 객체를 조작할 수 있다.
   join문법이 sql과는 다소 차이가 있지만 직접 쿼리를 작성할 수 있는 문법을 제공한다.
   주의할 점은 FROM절에 기술할 테이블명은 반드시 엔티티명이 작성되어야 한다.
     */
    @Test
    ...
    이때 필요한 것은 ManyTOOne어노테이션에 영속성 전이 설정을 해주는 것이다.
    영속성 전이랑 특정 엔티티를 영속화할 때 연관된 엔터티 도 함꼐 영속화한다는 의미이다.

    cascade-Cascade_PERSIST를 설정하면 MenuAndCategory엔터티를 영속화 할때 CAtegory엔터티도 함꼐 영속화 하게 된다.
     */
    @Test
    ...
    void 영속성_삭제_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,100016);
        ...
        transaction.commit();
        MenuAndCategory deletMenu = entityManager.find(MenuAndCategory.class,100016);
